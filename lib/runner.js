...
class runner {
  ...
  setupJasmine () {
    ...
    this.jas.loadConfigFile(path.resolve(this.configDir, 'jasmine.json'));
    this.jas.env.clearReporters();
    this.jas.env.addReporter(new SpecReporter(require(path.resolve(this.configDir, 'spec-reporter.json'))));
